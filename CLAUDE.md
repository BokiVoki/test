...
- **홈 화면(v2)**: `#home` 안이 `.homegrid`(본문 + 오른쪽 `.homeside` 보드, 900px 이하에선 세로로 쌓임).
  ...
  - **월 목표 · 꼭 이뤄야 할 것(2026-09-23)**: 월별 제목(`#monthTitle`) 바로 아래 `.monthgoal` — 🎯 월 목표(순수 텍스트 인풋 `#mgGoal`) + 번호 매긴 "꼭 이뤄야 할 것" 3칸(`.mgmust`, `data-mi="0|1|2"`). 저장은 **월간 회고와 같은 `journal` 행**(`date`='YYYY-MM', `jEntry(m)` 재사용)에 `goal`/`musts` 컬럼만 추가해서 같이 담음(마이그레이션 `add_journal_month_goal`, `hasMonthGoal` 프로브) — 굳이 새 테이블을 안 만들고 "그 달"을 나타내는 기존 journal 월간 행을 공유. 저널 탭의 월간 회고와 마찬가지로 **워크스페이스 구분 없이** 달 하나에 하나(journal 자체가 원래 ws 컬럼이 없어서 이 패턴을 그대로 따름). `renderMonthNav()`가 달 바뀔 때마다 `renderMonthGoal()`로 그 달 값을 다시 채워넣고(`state.month` 기준), 입력은 `jBody`와 같은 패턴으로 600ms 디바운스 + blur + pagehide/visibilitychange 시 즉시 저장(`mgSaveNow`). **입력 중엔 값을 덮어쓰지 않게**(`document.activeElement` 체크) 방어해둠.
...
- **Supabase**
  ...
  - 테이블: `todos`, `memos`, `settings`, `art_artists`, `art_works` (RLS `owner=auth.uid()`) + **v2 추가**: `progress`(진행 기록, `todo_id`·`date`·`text` — 덮어쓰지 않고 쌓임) · `routines`(상시업무 마스터) · `journal`(`date`가 `YYYY-MM`이면 그 달의 월간 회고, `goal`/`musts`엔 월 목표·꼭 이뤄야 할 것) · `links`(`pinned`면 홈 오른쪽 고정) · `agenda`(회의 안건). `todos`엔 `month`/`review`/`routine_id`/`rolled` 컬럼, `memos`엔 `pinned`(홈 오른쪽 고정, 2026-09-23), `journal`엔 `goal`(text)/`musts`(jsonb, 2026-09-23) 컬럼 추가. 전부 추가만 한 마이그레이션이라 기존 행은 그대로.
