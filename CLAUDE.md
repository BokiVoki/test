...
- **Supabase**
  ...
  - 테이블: `todos`, `memos`, `settings`, `art_artists`, `art_works` (RLS `owner=auth.uid()`) + **v2 추가**: `progress`(진행 기록, `todo_id`·`date`·`text` — 덮어쓰지 않고 쌓임) · `routines`(상시업무 마스터) · `journal`(`date`가 `YYYY-MM`이면 그 달의 월간 회고) · `links`(`pinned`면 홈 오른쪽 고정) · `agenda`(회의 안건). `todos`엔 `month`/`review`/`routine_id`/`rolled` 컬럼, `memos`엔 `pinned`(홈 오른쪽 고정, 2026-09-23) 컬럼 추가. 전부 추가만 한 마이그레이션이라 기존 행은 그대로.
