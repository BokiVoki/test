...
- **홈 화면(v2)**: `#home` 안이 `.homegrid`(본문 + 오른쪽 `.homeside` 보드, 900px 이하에선 세로로 쌓임).
  ...
  - **오른쪽 보드 3개**: **주머니**(프로젝트 없는 할일 = 기존 renderInbox 그대로, 280px에선 표 칸이 글자를 쪼개서 행을 flex로 풀어 [체크+제목] / [버튼들] 두 줄로 접음) · **회의 안건**(`agenda` 테이블 — 할일이 아니라 "회의 때 꺼낼 말"). 각 안건에 **담기**/**✕** — 안건이 정해지면 **담기**가 `AGENDA_PROJECT`(기본 `회의실행`) 프로젝트의 할일로 넘기고 안건 목록에선 뺌(중복 방지, 되돌리기 지원). 그 이름의 프로젝트가 없으면 담는 순간 자동으로 생김(`projectNames()`가 할일에서 뽑으므로 별도 생성 불필요). 처음엔 체크한 것을 한 번에 비우는 `정리` 버튼이었는데, 정해진 안건은 지우는 게 아니라 **할일로 넘어가야** 한다는 요청으로 바뀜. 체크(☐)는 "얘기 끝났지만 할 일은 아님" 표시로 남겨둠 · **링크**(`links`에서 `pinned`인 것만, ★는 서랍에서 꽂음)
...
- **저널(v2 2단계)**: `journal` 테이블(`date`·`body`). 왼쪽 날짜 목록 + 오른쪽 본문.
  ...
  - **홈 오른쪽 「오늘 한 줄」 보드**: 저널 탭까지 안 가도 홈에서 바로 그날 기록을 한 줄씩 붙이는 칸(`#todayLineIn`, `renderTodayLine`). **같은 `journal` 행을 저널 탭과 나눠 쓰는 구조** — 홈에서 Enter로 줄을 덧붙이고(덮어쓰지 않음), 저널 탭에서 고치면 홈에도 그대로 반영됨. 각 줄 ✕로 삭제(되돌리기 지원). 보드 순서는 **오늘 한 줄 → 주머니 → 회의 안건 → 링크**.
...
- **Supabase**
  ...
  - 테이블: `todos`, `memos`, `settings`, `art_artists`, `art_works` (RLS `owner=auth.uid()`) + **v2 추가**: `progress`(진행 기록, `todo_id`·`date`·`text` — 덮어쓰지 않고 쌓임) · `routines`(상시업무 마스터) · `journal`(`date`가 `YYYY-MM`이면 그 달의 월간 회고) · `links`(`pinned`면 홈 오른쪽 고정) · `agenda`(회의 안건). `todos`엔 `month`/`review`/`routine_id`/`rolled` 컬럼 추가. 전부 추가만 한 마이그레이션이라 기존 행은 그대로.
