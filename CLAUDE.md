...
- **v2 재구조 (2026-09, 진행 중)**: 2026년 업무 엑셀(월별 시트 + 상시업무 + 작년 회고)에서 일하는 방식을 읽고 구조를 다시 짬. 와이어프레임: https://claude.ai/artifact/M88d4ZgDmCMXTbHPbDPvtL
  ...
  - **3단계 예정**: 상시업무(`routines`) 마스터 + 매월 자동 복제 + 지난달 미완료 이월(`rolled`)
...
- **서랍(v2, 아직 1단계 상태)**: **링크 전체 관리**만 동작하고 상시업무 칸은 안내문만 있음(3단계 예정).
...
- **Supabase**
  ...
  - **프로브 플래그**: 새 컬럼·테이블도 기존 방식대로 `hasMonth`/`hasAgenda`/`hasLinks`로 확인 후 켜짐 — 마이그레이션 안 된 DB에서도 앱이 안 깨짐.
