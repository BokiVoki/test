...
- **디스플레이 폰트(2026-09-23)**: "앱 제목이랑 월별 제목이 좀 더 크고 재밌는 폰트였으면"이라는 요청 — 나머지 UI는 전부 시스템 산세리프(`--sans`) 그대로 두고, **딱 이 둘에만** 구글 폰트 **Do Hyeon**(`--display` 토큰)을 얹음: 헤더 `.brand`("하루", 22px, accent색) · 로그인 화면 `.authcard h1`("하루 🌿", 34px, accent색) · 홈 `.monthnav h2`("2026년 9월", 21px). `<head>`에 `fonts.googleapis.com` 링크 추가(`/cha`가 이미 같은 방식으로 Noto Serif KR을 쓰고 있어서 같은 패턴). 섹션 소제목(`.bhead h2` 등)이나 저널 날짜 제목(`.jhead h2`)은 요청 범위 밖이라 안 건드림 — 필요해지면 그때 넓히기.
