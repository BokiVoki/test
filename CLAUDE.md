...
- **`/cha` — 한강점 찻잎 도매 장부**(`webapp/cha.html`, 별도 정적 페이지): 하루앱과 완전히 분리된 단일 HTML. **로그인 없이 누구나 열림**(Supabase 안 씀, 데이터는 그 브라우저 `localStorage`에만). 탭 4개 = 장부(구매처 비교표·다과 단가표·다과 상 구성(품목+개수→상 원가 자동합산)·블렌딩 원가/마진 계산(찻잎+다과 상 합계 기준)·차종별 최적 우림법(블렌딩표에서 분리한 별도 표)·발주 요약, 각 표마다 CSV 다운로드 버튼) / 우림 가이드(차 30종 우림 스펙 — 포트·개완·다관·냉침 + 계산기) / 차 백과(중국·대만·일본·한국·동남아 품종표) / 향미 사전(향 계열표 아로마 휠 + 회감·구감·밀향 등 다도 용어 + 봉황단총 십대 향형). 원래 클로드 아티팩트로 만든 걸 옮긴 거라 **`window.storage`(아티팩트 전용 API)를 `localStorage` 셰임으로 교체**해야 저장이 동작함 — 나중에 아티팩트에서 새 버전을 가져올 때도 이 부분을 꼭 다시 갈아끼울 것. **구매처 비교표 항목은 안 바뀌는 `id`로 블렌딩 구성표와 연결**(`rowKey`) — 예전엔 "이름__구매처" 문자열로 연결해서 비교표에서 이름만 고쳐도 블렌딩표 선택이 조용히 "직접 입력·0원"으로 끊기는 버그가 있었음, 지금은 이름을 고쳐도 안 끊김(예전 데이터도 열 때 자동 이전). **표/백과 검색은 엑셀 찾기st**(`makeFinder`) — 행을 숨기지 않고 n/m 카운터로 순서대로 이동, input 값은 `rowInputText`로 따로 모아서 검색(`textContent`만 쓰면 `<select>`의 선택 안 된 옵션까지 걸려서 아무 분류명이나 검색해도 전체 행이 잡히는 버그가 있었음). 검색엔진 노출은 `<meta name="robots" content="noindex">`로 막아둠(단가가 공개돼 있어서). ⚠️ URL을 아는 사람은 누구나 도매 단가를 볼 수 있음. **회사 공유용이라 하루앱으로 돌아가는 링크는 일부러 안 넣음** — 이 페이지에 하루앱 흔적을 남기지 말 것. **표 칸 너비 드래그 조절**(`setupColResize`): 구매처 비교표·다과 단가·블렌딩 구성표·최적 우림법 표의 `<th>` 오른쪽 끝에 드래그 핸들이 있어 칸 너비를 직접 조절할 수 있고, 표 id별로 `localStorage`(`jamwon-tea-colw-{표id}`)에 저장돼 새로고침해도 그대로 유지됨(`table-layout:fixed`로 고정, 행이 늘거나 줄어도 — thead는 다시 안 그려지므로 — 너비 안 흐트러짐). **저장 안정성**: 입력 후 0.5초 뒤 저장하는 디바운스라 그 사이에 탭을 닫거나 다른 화면으로 전환하면 마지막 입력이 날아갈 수 있어서, 탭이 백그라운드로 가거나(`visibilitychange`) 페이지를 떠나거나(`pagehide`) 창 포커스를 잃을 때(`blur`) 대기 중인 저장을 즉시 실행하는 `flushAllSaves`를 추가함(비교표/다과단가/다과상구성/블렌딩표 4곳 전부).
