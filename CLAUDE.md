...
- **홈 화면(v2)**: `#home` 안이 `.homegrid`(본문 + 오른쪽 `.homeside` 보드, 900px 이하에선 세로로 쌓임).
  ...
  - **오른쪽 보드 3개**: **주머니**(프로젝트 없는 할일 = 기존 renderInbox 그대로, 280px에선 표 칸이 글자를 쪼개서 행을 flex로 풀어 [체크+제목] / [버튼들] 두 줄로 접음) · **회의 안건**(`agenda` 테이블 — 할일이 아니라 "회의 때 꺼낼 말", 체크 후 `정리`로 한 번에 비움) · **링크**(`links`에서 `pinned`인 것만, ★는 서랍에서 꽂음)
