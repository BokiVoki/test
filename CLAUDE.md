...
- **v2 재구조 (2026-09, 진행 중)**: 2026년 업무 엑셀(월별 시트 + 상시업무 + 작년 회고)에서 일하는 방식을 읽고 구조를 다시 짬. 와이어프레임: https://claude.ai/artifact/M88d4ZgDmCMXTbHPbDPvtL
  ...
  - **2단계 예정**: 진행 기록(`progress` 테이블, 쌓임 + 저널 자동 연동) + 저널 본체(날짜별 기록, 월말 회고)
...
- **홈 화면(v2)**: `#home` 안이 `.homegrid`(본문 + 오른쪽 `.homeside` 보드, 900px 이하에선 세로로 쌓임).
  ...
  - **월간 표**: `#monthTitle`+`‹ ›`+`이번 달로`(`#mThis`)로 달 이동(`goMonth`), 프로젝트 아코디언이 그 달 항목만 보여줌. 표 칸은 `체크 | 할일 | 회고 | 마감 | 예상 | 중요` — **회고**(`todos.review`)는 contenteditable이고 포커스 빠질 때 저장(예상시간 칸과 같은 방식), 680px 이하에선 숨김.
...
- **저널/서랍(v2, 1단계 상태)**: 저널(`#journal`)은 아직 **완료함 목록만** 보여주는 임시 상태(`doneList` 재사용), 서랍(`#drawer`)은 **링크 전체 관리**만 동작하고 상시업무 칸은 안내문만 있음.
